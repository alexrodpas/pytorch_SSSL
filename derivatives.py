...
def S_derivation(Z, X, W, G, Shape_t, Xij_skp, H_skp, SS, parameters):
    ...
    parameter3 = parameters['lambda_3'] * np.matmul(W, (np.matmul(W.T, X) - Z))     # parameter for weighted calculations
        ...
        for p in range(length):
            for i in range(mG):
                for j in range(nG):
                    ...
                    Gij_skp[i, j, k, p] = (-2 * G[i, j]/parameters['sigma']**2) * np.sum(np.multiply((X[:, i] - X[:, j]), (Xij_skp[:, i, p] - Xij_skp[:, j, p])))
            
            Part1[k, p] = np.sum(np.matmul(parameter1, Gij_skp[:, :, k, p]))    # Part 1: 1/2 Z^TZ * derivative
            Part2[k, p] = np.sum(np.matmul(parameter2[k, :], H_skp[k, :, p]))   # Part 2: weighted normalized shapelet derivatives
            Part3[k, p] = np.sum(np.matmul(parameter3, Xij_skp[k, :, p]))       # Part 3: shapelet derivatives
